...
describe("UniswapConnector", () => {
    ...
    before(async () => {
        ...
        await uniswapConnector.setUniswapProtocol(uniswapRouter.address, uniswapFactory.address, true);
        await uniswapConnector.setPrimitiveProtocol(trader.address, registry.address, true);
        ...
        await underlyingToken.connect(Admin).approve(uniswapConnector.address, MILLION_ETHER);
        await strikeToken.connect(Admin).approve(uniswapConnector.address, MILLION_ETHER);
        await optionToken.connect(Admin).approve(uniswapConnector.address, MILLION_ETHER);
    ...
    describe("mintAndMarketSell", () => {
        it("should mint then market sell", async () => {
            ...
            await expect(uniswapConnector.mintAndMarketSell(optionToken.address, ONE_ETHER, 1))
    ...
    describe("rollOptionsForExpiry()", () => {
        it("should roll option 1 (shorter expiry) to option 2 (longer expiry)", async () => {
            ...
            await longOptionToken.connect(Admin).approve(uniswapConnector.address, MILLION_ETHER);
            await shortRedeemToken.connect(Admin).approve(uniswapConnector.address, MILLION_ETHER);
